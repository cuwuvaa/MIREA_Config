...
def visualize_graph(plantuml_code, viz_tool):
    """
    Saves the PlantUML code to a temporary file, invokes the visualization tool, and displays the graph.
    """
    with tempfile.NamedTemporaryFile(mode='w+', suffix='.uml', delete=False) as f:
        f.write(plantuml_code)
        temp_uml_file = f.name
    # Assuming the visualization tool can read PlantUML files directly
    subprocess.run([viz_tool, temp_uml_file])
